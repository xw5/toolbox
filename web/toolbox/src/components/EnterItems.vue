<template>
  <ul class="flex flex-row flex-wrap justify-center list-none" v-if="tools.length > 0">
    <li 
      class="w-[268px] h-[80px] box-border flex-none mr-[5px] mb-[5px] bg-white shadow-[0px_0px_20px_-5px_rgba(158,158,158,.2)] cursor-pointer relative enter-item" 
      v-for="(tool, index) in tools" 
      :key="index"
    >
      <a 
        class="flex flex-row items-center w-full h-full box-border px-[10px] py-[5px] no-underline"
        :title="tool.title"
        :target="isOutLink(tool) ? '_blank' : '_self'" 
        :href="isOutLink(tool) ? tool.pathName || tool.url : 'javascript:void(0);'" 
        @click="isOutLink(tool) ? null : gotoPage(tool)">
        <img :src="tool.img" class="w-[64px] h-[64px] mr-[5px]" alt="">
        <div class="flex flex-col h-full justify-around">
          <h3 class="text-black text-[14px]">{{ tool.title }}</h3>
          <p class="text-[#6c757d] text-[12px] break-all">{{ tool.desc }}</p>
        </div>
        <div class="w-[16px] h-[16px] text-[#999] absolute top-[3px] right-[3px]" v-if="outInTag && !tool.urls">
 ...
 * @param tool 
 */
const isOutLink = (tool) => {
  if (tool.urls) {
    return false;
  }
  if (tool.url) {
    // 要求全部外部打开，或者它本身就要求外部打开的就全部外部唤起浏览器打开
    if (props.isOut || tool.isOut) {
      return true;
    } else {
      return false;
    }
